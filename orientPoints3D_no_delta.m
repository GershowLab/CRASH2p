function [at, pvec, at180, pvec180] = orientPoints3D_no_delta(x,y,z,w)
...
m1 = [mean(w.*(x2.^2)) mean(w.*x2.*y2);mean(w.*x2.*y2) mean(w.*(y2.^2))];
[uz,~,~] = svd(m1);
if (det(uz) < 0)
    uz = [1 0;0 -1]*uz;
end
...
%orient so it widens towards the right:
%sum(w.*(pts2(2,:)+dy).^2.*(pts2(1,:)+dx)) < 0 ?

%orient so y^3 moment is positive - makes the bulkier portion lower
if sum(w.*(pts2(2,:)+dy).^3) < 0
    uz = -uz;
    pts2 = uz*[x;y];
    dx = -mean(w.*pts2(1,:));
    dy = -mean(w.*pts2(2,:));
