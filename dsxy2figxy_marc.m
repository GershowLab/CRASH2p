...
%
%    figpos = dsxy2figxy_marc(axes_handle) -- gives position of box
%    bounding axes xlim and ylim
%
...
if (isempty(varargin))
    varargin{1} = [hAx.XLim(1) hAx.YLim(1) diff(hAx.XLim) diff(hAx.YLim)];
end
...
if exist('x','var')     % Transform a and return pair of points
    if (strcmpi(hAx.XScale, 'linear'))
    	varargout{1} = (x - axlim(1)) * axpos(3) / axwidth + axpos(1);
    else
        varargout{1} = (log(x/axlim(1))) * axpos(3) / log(axlim(2)/axlim(1)) + axpos(1);
    end
    if (strcmpi(hAx.YScale, 'linear'))
        varargout{2} = (y - axlim(3)) * axpos(4) / axheight + axpos(2);
    else
        varargout{2} = log(y/axlim(3)) * axpos(4) / log(axlim(4)/axlim(3)) + axpos(2);
    end
